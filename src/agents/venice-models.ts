  ...
  try {
    ...
    for (const apiModel of data.data) {
      ...
      if (catalogEntry) {
        ...
        // Use the model-reported maxCompletionTokens when available, with a
        // safety ceiling to prevent unbounded values from inflating costs or
        // destabilising token-budget logic (CWE-400).
        const VENICE_MAX_COMPLETION_CEILING = 131072;
        const rawMaxTokens = apiModel.model_spec.maxCompletionTokens;
        const maxTokens =
          typeof rawMaxTokens === "number" && Number.isFinite(rawMaxTokens) && rawMaxTokens > 0
            ? Math.min(rawMaxTokens, VENICE_MAX_COMPLETION_CEILING)
            : 4096;
